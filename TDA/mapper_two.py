...
mapper.fit(X=np.arange(n_tf)[:,None], filters=eigenvectors[:,0:2], colors=np.hstack([n_targets_tf[:,None], A_tf_filtered[:,1:]])) #n_targets_tf[:,None])
...
# plt.savefig('../Results/Clean_plot/Filter_function_activity.svg', format='svg')
plt.show()
...
plt.ylabel('New Filter')
# plt.savefig('../Results/Clean_plot/Filter_function_edge.svg', format='svg')
plt.show()
